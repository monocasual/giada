...
void SamplePlayer::render(ChannelShared& shared) const
{
	...
	Frame& generated = shared.generated;

	/* Compute the number of frames expected to be generated by the WaveReader
	and it's internal Resampler for the current	begin-end range. */

	const Frame generable = (end - begin) / pitch;

	/* If rewinding, fill the audio buffer normally first, then reset the tracker 
	to the begin point. The rest is performed as usual: the second call to
	fillBuffer will overwrite the buffer content at the given offset with audio
	data coming from 'begin' point:
	
	audioBuffer, 1st call = [abcdefghijklmnopqrs]
	audioBuffer, 2nd call = [abcdefghijkl|abcdef]
	                                     ^-- offset    */

	if (shared.rewinding)
	{
		if (generated < generable) // If there's more data to read
		{
