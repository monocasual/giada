...
void ChannelManager::loadSampleChannel(ID channelId, Wave& wave)
{
	...
	Wave&       newWave = wave;
	const Wave* oldWave = channel.sampleChannel->getWave(0);
...
void ChannelManager::setBeginEnd(ID channelId, Frame b, Frame e)
{
	...
	b = std::clamp(b, 0, c.sampleChannel->getWaveSize(0) - 1);
	e = std::clamp(e, 1, c.sampleChannel->getWaveSize(0));
	if (b >= e)
		b = e - 1;
	else if (e < b)
		e = b + 1;
...
void ChannelManager::resetBeginEnd(ID channelId)
{
	...
	c.sampleChannel->end   = c.sampleChannel->getWaveSize(0);
	m_model.swap(model::SwapType::HARD);
}
...
void ChannelManager::loadWaveInPreviewChannel(ID channelId)
{
	...
	previewCh.loadWave(sourceCh.sampleChannel->getWave(0));
