...
void Document::load(const Patch& patch, Shared& shared, float sampleRateRatio)
{
	...
	for (const Patch::Track& ptrack : patch.tracks)
	{
		...
		{
			const auto it = u::container::findIf(patch.channels, channelId);
			...
			{
				return Sample{
				    shared.findWave(ps.waveId), ps.range, ps.shift, ps.pitch, ps.time, ps.playbackMode};
			});
	...
	sequencer.setTimeSignature({patch.beats, patch.bars});
...
{
	patch.bars      = sequencer.getTimeSignature().bars;
	patch.beats     = sequencer.getTimeSignature().beats;
