...
: channelId(c.id)
, channelType(c.type)
, enabled(c.midiLearn.enabled)
, velocityAsVol(c.sampleChannel ? c.sampleChannel->velocityAsVol : 0)
, filter(c.midiLearn.filter)
, keyPress(c.midiLearn.keyPress.getValue())
, keyRelease(c.midiLearn.keyRelease.getValue())
, kill(c.midiLearn.kill.getValue())
, arm(c.midiLearn.arm.getValue())
, volume(c.midiLearn.volume.getValue())
, mute(c.midiLearn.mute.getValue())
, solo(c.midiLearn.solo.getValue())
, pitch(c.midiLearn.pitch.getValue())
, readActions(c.midiLearn.readActions.getValue())
{
